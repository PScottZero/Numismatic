...
enum class CoinType(
    ...
    fun greensheetType(): String {
        return this.type ?: "${this}s"
    }

    fun greensheetProof(): String {
        return "${this.greensheetType()} (Proof)"
    }

    companion object {
        fun fromString(coinType: String): CoinType? {
            values().forEach {
                if (it.toString() == coinType) return it
            }
            return null
        }
    }
